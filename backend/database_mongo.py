...
class MongoDBManager:
    ...
    @staticmethod
    def find_document(collection_name, query: dict = None, sort=None, projection: dict | None = None):
        """Find a single document in a collection.

        Accepts optional `sort` (e.g. [("created_at", -1)]) and `projection`.
        """
        col = MongoDBManager.get_collection(collection_name)
        q = query or {}
        # pymongo.find_one accepts sort and projection kwargs
        if sort and projection is not None:
            return col.find_one(q, projection=projection, sort=sort)
        if sort:
            return col.find_one(q, sort=sort)
        if projection is not None:
            return col.find_one(q, projection=projection)
        return col.find_one(q)

    @staticmethod
    def find_documents(collection_name, query: dict | None = None, sort=None, limit: int | None = None, skip: int | None = None):
        """Find multiple documents in a collection.

        Added `skip` parameter to support pagination.
        """
        col = MongoDBManager.get_collection(collection_name)
    ...
    @staticmethod
    def update_documents(collection_name, query: dict, update_data: dict) -> int:
        """Update multiple documents in a collection. Returns number of modified documents."""
        col = MongoDBManager.get_collection(collection_name)
        # Don't add updated_at for bulk updates as they might use $set operator
