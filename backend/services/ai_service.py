...
class AIService:
    ...
    @staticmethod
    def generate_description(title: str, keywords: List[str] = None) -> Dict:
        """
        Generate an AI-suggested description based on task title and keywords.
        Returns suggested lines based on the task title and keywords.
        When keywords like 'frontend', 'backend', 'fullstack', 'hardware' are provided,
        generates more elaborate and detailed content specific to those areas.
        """
        try:
            ...
            normalized_keywords = [k.lower().replace("-", " ").replace("_", " ") for k in (keywords or [])]
            keyword_set = set(normalized_keywords)
            
            # Detect category keywords
            has_frontend = any(k in keyword_set for k in ['front', 'frontend', 'front end', 'front-end'])
            has_backend = any(k in keyword_set for k in ['back', 'backend', 'back end', 'back-end'])
            has_fullstack = any(k in keyword_set for k in ['fullstack', 'full stack', 'full stack', 'full-stack'])
            has_hardware = any(k in keyword_set for k in ['hardware', 'circuit', 'sensor', 'embedded', 'electronics'])
            
            # Check if OpenAI is configured
            api_key = os.getenv("OPENAI_API_KEY")
            if not api_key:
                # Fallback: return enhanced description based on keywords
                keyword_text = f" (including keywords: {', '.join(keywords)})" if keywords else ""
                
                # Generate enhanced fallback based on keywords
                if has_fullstack:
                    description = f"Full Stack Development Task: {eng_title}\n\nThis is a full-stack development task that requires both frontend and backend implementation."
                    lines = [
                        f"Frontend: Design and implement the user interface for {eng_title.lower()}",
                        f"Backend: Develop server-side logic and APIs for {eng_title.lower()}",
                        "Integrate frontend with backend services",
                        "Test and verify both frontend and backend functionality"
                    ]
                elif has_frontend and not has_backend:
                    description = f"Frontend Development Task: {eng_title}\n\nThis is a frontend-focused task requiring user interface implementation."
                    lines = [
                        f"Design the user interface for {eng_title.lower()}",
                        "Implement responsive and accessible UI components",
                        "Ensure cross-browser compatibility",
                        "Optimize frontend performance"
                    ]
                elif has_backend and not has_frontend:
                    description = f"Backend Development Task: {eng_title}\n\nThis is a backend-focused task requiring server-side implementation."
                    lines = [
                        f"Design and implement server-side logic for {eng_title.lower()}",
                        "Create and optimize database schemas",
                        "Develop RESTful APIs",
                        "Implement security best practices"
                    ]
                elif has_hardware:
                    description = f"Hardware Development Task: {eng_title}\n\nThis is a hardware-focused task requiring physical components implementation."
                    lines = [
                        f"Design hardware components for {eng_title.lower()}",
                        "Select appropriate sensors and microcontrollers",
                        "Create circuit diagrams and PCB layouts",
                        "Test and validate hardware functionality"
                    ]
                else:
                    description = f"Task: {eng_title}{keyword_text}"
                    lines = [
                        f"Complete {eng_title.lower()}",
                        "Ensure quality and attention to detail",
                        "Document progress and updates"
                    ]
            ...
            # Use OpenAI to generate description with enhanced prompts
            try:
                from openai import OpenAI
                os.environ["OPENAI_API_KEY"] = api_key
                client = OpenAI()
...
- Include frontend components (UI/UX, responsive design, user interactions)
- Include backend components (API design, database, server logic)
- Include integration aspects (connecting frontend to backend)
- Cover both sides comprehensively"""
                elif has_frontend and not has_backend:
                    category_guidance = """
CATEGORY: Frontend Development
- Focus on user interface and user experience
- Include responsive design considerations
- Mention UI frameworks, styling, and component design
- Consider accessibility and performance"""
                elif has_backend and not has_frontend:
                    category_guidance = """
CATEGORY: Backend Development
- Focus on server-side logic and data handling
- Include API design and database considerations
- Mention security, performance optimization
- Cover authentication and authorization if relevant"""
                elif has_hardware:
                    category_guidance = """
CATEGORY: Hardware Development
- Include circuit design and components selection
- Mention sensors, microcontrollers, or embedded systems
- Cover hardware-software integration if applicable
- Include testing and validation of physical components"""

                keyword_text = f"Keywords to include: {', '.join(keywords)}\n" if keywords else ""
                prompt = (
                    "Generate a detailed task description with 5-6 actionable lines based on the task title.\n"
                    "Return ONLY valid JSON with keys: description (string), lines (array of strings).\n"
                    "Make the description practical, specific, and elaborate.\n"
                    "IMPORTANT: If keywords are provided, make sure to incorporate them naturally into the description and action items.\n"
                    "For 'fullstack' tasks, provide both frontend AND backend action items.\n"
                    "For 'frontend' tasks, focus on UI/UX and user-facing aspects.\n"
                    "For 'backend' tasks, focus on server-side and data aspects.\n"
                    "For 'hardware' tasks, focus on physical components and electronics.\n"
                    f'{category_guidance}\n'
                    f'Task Title: "{eng_title}"\n'
                    f"{keyword_text}"
                )
                ...
                description = parsed.get("description", f"Task: {eng_title}")
                ...
                return {
                    "suggested_description": description,
                    "suggested_lines": lines[:6]  # limit to 6 lines for more detail
                }

            except Exception as openai_err:
                print(f"[AIService][generate_description] OpenAI error: {openai_err}")
                # Fallback with enhanced keyword handling
                keyword_text = f" (including keywords: {', '.join(keywords)})" if keywords else ""
                
                if has_fullstack:
                    return {
                        "suggested_description": f"Full Stack Task: {eng_title}{keyword_text}\n\nThis is a full-stack task requiring both frontend and backend work.",
                        "suggested_lines": [
                            f"Frontend: Build UI for {eng_title.lower()}",
                            f"Backend: Implement server logic for {eng_title.lower()}",
                    ...
                    return {
                        "suggested_description": f"Frontend Task: {eng_title}{keyword_text}\n\nThis is a frontend task focusing on user interface.",
                        "suggested_lines": [
                            f"Design UI for {eng_title.lower()}",
                    ...
                    return {
                        "suggested_description": f"Backend Task: {eng_title}{keyword_text}\n\nThis is a backend task focusing on server-side logic.",
                        "suggested_lines": [
                            f"Implement server logic for {eng_title.lower()}",
                    ...
                    return {
                        "suggested_description": f"Hardware Task: {eng_title}{keyword_text}\n\nThis is a hardware task focusing on physical components.",
                        "suggested_lines": [
                            f"Design hardware for {eng_title.lower()}",
                            "Select components",
                    ...
                    return {
                        "suggested_description": f"Task: {eng_title}{keyword_text}",
                        "suggested_lines": [
                            f"Complete {eng_title.lower()}",
                            "Ensure quality and attention to detail",
                            "Document progress and updates"
            ...
            print(f"[AIService][generate_description] Error: {e}")
            return {
                "suggested_description": f"Task: {title}",
                "suggested_lines": [
                    "Research and understand the task",
                    "Plan the approach",
                    "Execute and document progress",
                    "Review and finalize"
                ]
            }
