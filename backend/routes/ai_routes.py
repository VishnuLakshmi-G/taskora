...
def generate_description():
    """Generate AI-suggested description based on task title and keywords."""
    try:
        data = request.get_json() or {}
        ...
        # Use the dedicated AIService method
        res = AIService.generate_description(title, keywords)

        return jsonify({
            "suggested_description": res.get("suggested_description", ""),
            "suggested_lines": res.get("suggested_lines", [])
        }), 200
