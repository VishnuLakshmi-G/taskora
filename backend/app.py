...
def create_app() -> Flask:
    ...
    # JWT token expiration (7 days in production, no expiration in dev)
    ...
    if os.getenv("FLASK_ENV") == "production":
        ...
        allowed_origins = os.getenv("ALLOWED_ORIGINS", "https://genai-task-manager-frontend.onrender.com").split(",")
        CORS(
            app,
            resources={
                r"/api/*": {
...
from routes.tester_routes import tester_bp
...
app.register_blueprint(tester_bp, url_prefix='/api/tester')
...
# Start scheduler & run
# ----------------------------
# Start the Reminder + WhatsApp Scheduler safely (optional for basic deployment)
try:
    ...
    print(f"[Scheduler] Failed to start (continuing without scheduler): {e}")
