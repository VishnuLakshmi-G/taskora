...
const TaskList = ({ refreshTrigger, onTaskUpdate }) => {
  ...
  useEffect(() => {
    ...
    const handleTaskStatusChanged = (event) => {
      console.log('[TaskList] Task status changed by admin:', event.detail);
  ...
  const handleTaskAction = async (taskId, action, data = {}) => {
    try {
      ...
      if (response && response.task) {
        ...
        // Notify other clients (e.g., admin dashboard) when a task is completed.
        if (response.task.status === "completed") {
          window.dispatchEvent(new CustomEvent("taskStatusChanged", {
            detail: { taskId, status: response.task.status },
          }));
        }
      }
